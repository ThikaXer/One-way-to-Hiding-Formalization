...
proof -
  have "(\<lambda>y. c *\<^sub>C f y) summable_on A"
    using summable_on_scaleC_right has_sum_imp_summable[OF assms] by auto
  then show ?thesis unfolding summable_iff_has_sum_infsum infsum_scaleC_right
...
by (simp add: assms trace_class_comp_right)
